...
### Persona Components
- Avatar: 3D avatar that is visualized in a web browser - this is done using the code in the incarnation folder
- Actions: LLM Responses used to personify the Persona
- Voice: Optional Voice that will be sourced from a TTS model - this is done using a docker container created by voice_generation folder
...
- Personas should have a clear distinction between roleplaying and there actual actions. (I.E given access to a file there personality would want to delete. We need to ensure this does not happen.) I would want this reenforced by the LLM prompt. Maybe some kind of safety check before using tools.(Future)
### Presona Stimulus
...
- PlayAIdes will communicate with LLM(s) and tts models as well as send data to the avatar and voice components using http /websockets. 
- PlayAIdes will direct all persona related services these are all optional :
  - Incaration services
  - LLM services
  - TTS services
